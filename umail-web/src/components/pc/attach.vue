<template>
  <div class="ums-page">
    ...
    <div class="ums-container">
      ...
      <div class="ums-main">
        <el-table
          :data="messagesWithAchment"
          style="width: 100%;font-size: 0.7rem;"
          :show-header="true"
          :highlight-current-row="true"
          :default-sort="{ prop: 'date', order: 'descending' }"
        >
          <el-table-column label="附件名" prop="name" sortable>
            <template slot-scope="scope">
              <el-tooltip
                effect="light"
                :content="scope.row.attachment[0].name"
                placement="top"
              >
                <span class="ums-ellipsis">{{
                  scope.row.attachment[0].name
                }}</span>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column label="所在邮件" prop="inMsg" sortable>
            <template slot-scope="scope">
              <el-tooltip
                effect="light"
          ...
          <el-table-column label="发件人" prop="from" sortable>
            <template slot-scope="scope">
              <el-tooltip
                effect="light"
                :content="scope.row.from[0]"
                placement="top"
              >
                <span class="ums-ellipsis">{{
                  scope.row.from[0].substring(
                    scope.row.from[0].indexOf("<") + 1,
                    scope.row.from[0].indexOf(">")
                  )
                }}</span>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column label="发件日期" prop="date" sortable>
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" prop="operator">
            <template slot-scope="scope">
              <a
                ...
                :href="
                  base_url + '/download?attach=' + scope.row.attachment[0].path
                "
                >下载</a
              >
              <span>|</span>
              <span
                class="attach-operator"
                @click="forwardAttach"
                :data-attach="JSON.stringify(scope.row.attachment[0])"
                >转发</span
              >
            </template>
          </el-table-column>
  ...
  methods: {
    ...
    saveCurrentAttachment() {
      ...
      sessionStorage.setItem(
        "currentAttachment",
        JSON.stringify(_currentAttachment)
      );
    },
