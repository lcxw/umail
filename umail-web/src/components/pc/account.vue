      ...
      <div class="ums-aside" style="width:480px;">
        ...
        <el-table :data="accounts" :show-header="false">
          ...
          <el-table-column fixed="right" label="操作" width="50">
            <template slot-scope="scope">
              <el-button @click="newAccount" type="text" size="small"
                >新建</el-button
              >
            </template>
          </el-table-column>
      ...
      <div class="ums-main">
        ...
        <div v-if="status == 'update'">
              ...
              >
                <el-form-item label="邮箱类型:">
                  <span style="color: #606266;">{{
                    account.recprotocol.toLocaleUpperCase()
                  }}</span>
                </el-form-item>
                <el-form-item label="收件服务器:">
                  <el-input
                    v-model="account.rechost"
                    size="small"
                    style="width:200px"
                  ></el-input>
                  <el-checkbox
                    v-model="account.recisssl"
                    @change="accountRecSSLChange"
                    style="margin-left:6px;margin-right:6px;color: #606266;"
                    >SSL</el-checkbox
                  >
                  <span style="color: #606266;">端口：</span
                  ><el-input
                    v-model="account.recport"
                    size="small"
                    style="width:70px"
                  ></el-input>
                </el-form-item>
                <el-form-item label="发件服务器:">
                  <el-input
                    v-model="account.sendhost"
                    size="small"
                    style="width:200px"
                  ></el-input>
                  <el-checkbox
                    v-model="account.sendisssl"
                    @change="accountSendSSLChange"
                    style="margin-left:6px;margin-right:6px;color: #606266;"
                    >SSL</el-checkbox
                  >
                  <span style="color: #606266;">端口：</span
                  ><el-input
                    v-model="account.sendport"
                    size="small"
                    style="width:70px"
                  ></el-input>
                </el-form-item>
            ...
            <el-tab-pane label="设置" name="third">
              ...
              >
                <el-form-item label="定时收取:">
                  <el-checkbox
                    v-model="account.isscheduled"
                    @change="accountIsScheduledChange"
                ...
                <el-form-item label="时间间隔:">
                  <el-input
                    v-model="account.scheduledperiod"
                    size="small"
                    :disabled="!account.isscheduled"
                    style="width:60px"
                  ></el-input>
          ...
          <div style="position:absolute;right:40px;bottom:40px;">
            <el-button
              ...
              "
              @click="doModifyAccount"
              >确定</el-button
            >
            <el-button size="small" style="width:100px;" @click="$router.go(-1)"
              >取消</el-button
            >
        ...
        >
          <el-form-item label="接收服务器类型:">
            <el-select
              v-model="newaccount.recprotocol"
              size="small"
              style="width:200px"
              @change="newAccountRecSSLChange"
            >
              <el-option
          ...
          <el-form-item
            :label="
              newaccount.recprotocol.toLocaleUpperCase().concat('服务器:')
            "
          >
            <el-input
              v-model="newaccount.rechost"
              size="small"
              style="width:200px"
            ></el-input>
            <el-checkbox
              v-model="newaccount.recisssl"
              @change="newAccountRecSSLChange"
              style="margin-left:6px;margin-right:6px;color: #606266;"
              >SSL</el-checkbox
            >
          ...
          <el-form-item label="SMTP服务器:">
            <el-input
              v-model="newaccount.sendhost"
              size="small"
              style="width:200px"
            ></el-input>
            <el-checkbox
              v-model="newaccount.sendisssl"
              @change="newAccountSendSSLChange"
              style="margin-left:6px;margin-right:6px;color: #606266;"
              >SSL</el-checkbox
            >
            <span style="color: #606266;">端口：</span
            ><el-input
              v-model="newaccount.sendport"
              size="small"
              style="width:70px"
            ></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    doNewAccount() {
      ...
      this.$axios
        .post(
          localStorage.getItem("base_url") + "/accounts",
          _this.newaccount,
          {
            headers: {
              "Content-Type": "application/json",
    ...
    deleteAccount(row) {
      ...
      this.$confirm("您确认要永久删除" + row.account + "吗？", "确认", {
        ...
        .then(() => {
          _this.$axios
            .post(localStorage.getItem("base_url") + "/accounts", row, {
              headers: {
                "Content-Type": "application/json",
                token: localStorage.token,
                type: "delete"
    ...
    addAccount() {
      var _this = this;
      this.$axios
        .post(localStorage.getItem("base_url") + "/accounts", this.account, {
          headers: {
            "Content-Type": "application/json",
            token: localStorage.token,
            type: "save"
    ...
    accountRecSSLChange(data) {
      ...
      this.account.recport = data
        ? this.account.recprotocol == "imap"
          ? "993"
          : "995"
        : this.account.recprotocol == "imap"
        ? "143"
    ...
    newAccountRecSSLChange(data) {
      console.log("rec" + data);
      this.newaccount.recport = data
        ? this.newaccount.recprotocol == "imap"
          ? "993"
          : "995"
        : this.newaccount.recprotocol == "imap"
        ? "143"
      ...
      if (data.indexOf("@") != -1) {
        ...
        this.servers.forEach(function(item, index) {
          if (item.name == host) {
            ...
            _this.newaccount.sendisssl =
              item.sendisssl == "true" ? true : false;
          }
        });
      }
    }
