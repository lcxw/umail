<template>
  <div class="ums-page">
    <div ref="header" class="ums-header">
      <div style="width:74%">
        ...
        >
          <el-submenu index="1">
            <template slot="title"
              ><span class="iconfont umail-receive-mail menu-padding"></span
              >收取
            </template>
          ...
          <el-submenu index="2">
            <template slot="title"
              ><span class="iconfont umail-write-mail menu-padding"></span
              >写邮件
            </template>
          ...
          <el-submenu index="3" :disabled="currentMessageNum == -1">
            <template slot="title"
              ><span class="iconfont umail-reply-mail menu-padding"></span>回复
            </template>
          ...
          <el-submenu index="4" :disabled="currentMessageNum == -1">
            <template slot="title"
              ><span class="iconfont umail-replyall-mail menu-padding"></span
              >回复全部
            </template>
          ...
          <el-menu-item index="5" :disabled="currentMessageNum == -1"
            ><span class="iconfont umail-froward-mail  menu-padding"></span>转发
          </el-menu-item>
          ...
          <el-menu-item index="6" :disabled="currentMessageNum == -1"
            ><span class="iconfont umail-delete-mail  menu-padding"></span>删除
          </el-menu-item>
          <el-submenu index="7">
            <template slot="title"
              ><span class="iconfont umail-shezhi menu-padding"></span>管理
            </template>
            <el-menu-item index="7-1"
              ><span
                class="iconfont umail-attachment-manage submenu-padding"
              ></span
              >附件管理
            </el-menu-item>
            <el-menu-item index="7-2"
              ><span class="iconfont umail-sign-manager submenu-padding"></span
              >签名管理
            </el-menu-item>
            ...
            <el-menu-item index="7-4"
              ><span
                class="iconfont umail-contact-manager submenu-padding"
              ></span
              >联系人管理
            </el-menu-item>
            <el-menu-item index="7-5"
              ><span
                class="iconfont umail-account-manager submenu-padding"
              ></span
              >账号管理
            </el-menu-item>
            <el-menu-item index="7-6"
              ><span class="iconfont umail-logout submenu-padding"></span>退出
            </el-menu-item>
          </el-submenu>
    ...
    <div
      class="search-items"
      :style="{ display: searchInput.trim() != '' ? 'flex' : 'none' }"
    >
      <div
        class="ums-ellipsis search-item"
        @click="searchMsgByOrigin"
        data-origin="from"
      >
        发件人：{{ searchInput }}
      </div>
    ...
    <ums-loading
      ...
      :visible="syncLoadingVisibled"
    ></ums-loading>
    <ums-loading
      msg="正在删除邮件"
      value=""
      :visible="syncDeletingVisibled"
    ></ums-loading>
      ...
      <div class="ums-container">
        <div class="ums-aside" style="width:300px;">
          <el-table
            :data="messages"
            :height="620"
            @row-click="handleTableColumnClick"
          ...
          >
            <el-table-column width="30">
              <template slot-scope="scope">
                <span
                  class="iconfont umail-yuandian-copy"
                  style="color:#409EFF;font-size:30px;line-height: 1;"
                  v-if="scope.row.new == true"
                ></span>
                <span
                  class="iconfont umail-yuandian-copy"
                  style="color:#c2c2c2;font-size:30px;line-height: 1;"
                  v-else
                ></span>
              </template>
            </el-table-column>
            <el-table-column label="主题" width="150" sortable>
              <template slot-scope="scope">
        ...
        <div class="ums-main">
          <div v-if="currentMessageNum != -1 && messages.length != 0">
            ...
            <div class="message-header">
              <p
                class="message-header-subject"
                v-html="messages[currentMessageNum].subject"
              ></p>
              <p class="message-header-item">
                发件人：
                <el-popover
                  placement="bottom"
                  width="300"
                  trigger="hover"
                  v-for="(address, key) in messages[currentMessageNum].from"
                  :key="key"
                >
                  <div style="cursor:pointer;">
                    ...
                    <el-row :gutter="20" type="flex" align="middle">
                      <el-col :span="24">
                        <span style="fong-size:0.3em;color:#999999;">{{
                          address.substring(
                            address.indexOf("&lt;") + 1,
                            address.indexOf("&gt;")
                          )
                    ...
                    >
                      <el-col :span="12">
                        <div
                          style="text-align: center;color:#409EFF;border-top:1px solid #e0e0e0;border-right: 1px solid #e0e0e0;padding-top:10px;"
                        >
                          <span
                            @click="showContactDialog"
                            :data-address="address"
                            v-if="
                              contactEmails.indexOf(
                                address.substring(
                                  address.indexOf('<') + 1,
                                  address.indexOf('>')
                                )
                            ...
                            >添加到联系人</span
                          >
                          <span v-else style="color:#e0e0e0;"
                            >添加到联系人</span
                          >
                        </div>
                      </el-col>
                      <el-col :span="12">
                        <div
                          style="text-align: center;color:#409EFF;border-top:1px solid #e0e0e0;padding-top:10px;"
                        >
                          <span
                            @click="searchMsgByAddress"
                            :data-address="
                              address.substring(
                                address.indexOf('<') + 1,
                                address.indexOf('>')
                  ...
                  <span class="message-header-address" slot="reference">{{
                    address
                  }}</span>
                </el-popover>
              </p>
              <p class="message-header-item">
                收件人：<span
                  class="message-header-address"
                  v-for="address in messages[currentMessageNum].to"
                  >{{ address }}</span
                >
              </p>
              <p
                v-if="messages[currentMessageNum].cc.length != 0"
                class="message-header-item"
              >
                &nbsp;&nbsp;&nbsp;&nbsp;抄送：<span
                  v-for="address in messages[currentMessageNum].cc"
                  class="message-header-address"
                  >{{ address }}</span
                >
              </p>
              <p
                v-if="messages[currentMessageNum].bcc.length != 0"
                class="message-header-item"
              >
                &nbsp;&nbsp;&nbsp;&nbsp;密送：<span
                  v-for="address in messages[currentMessageNum].bcc"
                  class="message-header-address"
                  >{{ address }}</span
                >
              </p>
              <p class="message-header-item">
                &nbsp;&nbsp;&nbsp;&nbsp;时间：{{
                  messages[currentMessageNum].date
                }}
              </p>
            </div>
            <hr />
            ...
            <div class="message-header-attachs">
              <div
                class="message-header-attach"
                v-for="attach in attachmentWithUrls"
              >
                <a class="attach" :href="attach.url"
                  ><i class="el-icon-document menu-padding"></i
                  >{{ attach.name }}</a
                >
              </div>
            </div>
          ...
          <div v-else style="font-size:40px;color:#84898a;text-align: center;">
            {{ currentAccount }}
          </div>
        </div>
      </div>
...
export default {
    ...
    this.$axios({
      ...
      if (_accounts.length == 0) {
        ...
        var _accountNode = {
          ...
          value: "",
          children: [
            { label: "收件箱", value: "INBOX", children: [] },
  ...
  methods: {
    loadInboxMessage() {
      ...
      this.$axios({
        ...
        if (_this.currentMessageNum != -1 && _this.messages.length != 0) {
          _this.content =
            _this.messages[_this.currentMessageNum].content[0].text;
        }
      ...
      _this
        .$axios({
          ...
          url:
            ...
            _this.currentBox +
            "&msgUid=" +
            _this.messages[_this.currentMessageNum].uid,
          headers: {
            "Content-Type": "application/json",
            token: localStorage.token
          }
    ...
    searchMessages() {
      ...
      this.$axios({
        ...
        _messages.forEach(function(item, index) {
          ...
          item.content[0].text = _content.replace(
            reg,
            "<font color='red'>" + _this.searchInput + "</font>"
          );
        });
        ...
        _this.$message({
          type: "success",
          message: "查询成功，共查到" + _this.messages.length + "封信件！"
        });
      });
    },
    searchMsgByAddress(event) {
      ...
      this.$axios({
        ...
        url:
          ...
          "/search0?account=" +
          _this.currentAccount +
          "&origin=from&queryKey=" +
          event.target.dataset.address,
        headers: {
          "Content-Type": "application/json",
          token: localStorage.token
        }
    ...
    showContactDialog(event) {
      ...
      this.newcontact.email = _address.substring(
        _address.indexOf("<") + 1,
        _address.indexOf(">")
      );
    ...
    handleTableColumnClick(data) {
      ...
      if (_this.currentBox == "草稿箱") {
        ...
        console.log(
          _this.currentMessageNum +
            "/" +
            _this.messages[_this.currentMessageNum].new
        );
        console.log(_this.messages[_this.currentMessageNum]);
