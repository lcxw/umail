<template>
	<div  class="ums-page">
		...
		<ums-loading msg="正在发送邮件" :value="msg.subject" :visible="sendLoadingVisibled"></ums-loading>
		<el-row :gutter="20" style="height:60px;padding-top: 20px;padding-bottom: 10px;padding-left:20px;padding-right:20px;overflow: hidden;">
			<el-col :span="2">
				<div>
				<el-button type="primary"  :disabled="msg.to.length==0" size="small" @click="sendMessage"><span class="iconfont umail-send menu-padding"></span>发送</el-button>
				</div>
			</el-col>
		 	<el-col :span="2" v-if="way=='send'">
				<div>
				<el-button  type="primary" size="small" @click="saveToDraft"><span class="iconfont umail-save menu-padding"></span>保存</el-button>
				</div>
			</el-col>
			<el-col :span="2">
				<div>
						...
						:file-list="fileList">
						<el-button size="small" type="primary"><span class="iconfont umail-attachment menu-padding"></span>附件</el-button>
					</el-upload>
				</div>
			</el-col>
			<el-col :span="18">
		...
		<!--输入input-->
		<el-row :gutter="20" style="padding-top:10px;border-bottom: 1px solid #e5e5e5;padding-left:20px;padding-right:20px;">
		  <el-col :span="2">
		  ...
		  <el-col :span="22">
				<div style="display: flex;">
					<input style="width:300px;border: 0px;outline:none;" placeholder=""
						 v-model="toInput"  @change="toInputChange" :autofocus="true">
					</input>
					<div style="width:100%;">
					...
					<span class="iconfont umail-add return-row-padding" style="cursor: pointer;color:#409EFF;font-size:25px;"
					 @click="showContactDialog" data-to="to"></span>
				</div>
		  </el-col>
		</el-row>
		<el-row :gutter="20" style="padding-top:10px;border-bottom: 1px solid #e5e5e5;padding-left:20px;padding-right:20px;">
		  <el-col :span="2">
			...
			<el-col :span="22">
				<div style="display: flex;">
						<input style="width:300px;border: 0px;outline:none;" placeholder=""
						 v-model="ccInput"  @change="ccInputChange">
					</input>
					<div style="width:100%;">
						<el-tag style="margin-right:10px;border-radius: 20px;"
					...
					<span class="iconfont umail-add return-row-padding" style="cursor: pointer;color:#409EFF;font-size:25px;"
					 @click="showContactDialog" data-to="cc"></span>
				</div>
			</el-col>
		</el-row>
		<el-row :gutter="20" style="padding-top:10px;border-bottom: 1px solid #e5e5e5;padding-left:20px;padding-right:20px;">
		  ...
		  <el-col :span="22">
		  	<div style="display: flex;">
		  			<input style="width:300px;border: 0px;outline:none;" placeholder=""
		  			 v-model="bccInput"  @change="bccInputChange">
		  		</input>
		  		<div style="width:100%;">
		  			<el-tag style="margin-right:10px;border-radius: 20px;"
				...
				<span class="iconfont umail-add return-row-padding" style="cursor: pointer;color:#409EFF;font-size:25px;"
				  @click="showContactDialog" data-to="bcc"></span>
		  	</div>
		  </el-col>
		</el-row>
		<el-row :gutter="20" style="padding-top:10px;margin-bottom:10px; border-bottom: 1px solid #e5e5e5;padding-left:20px;padding-right:20px;">
		  <el-col :span="2">
				<div style="color:#84898a;">
				<span>主题：</span>
				</div>
		  ...
		  <el-col :span="22">
				<div style="display: flex;">
					<input style="width:100%;border: 0px;outline:none;" placeholder=""
						 v-model="msg.subject"  @change="subjectInputChange">
					</input>
				</div>
		  </el-col>
		</el-row>
		...
		<el-row :gutter="20" style="margin-bottom:10px;">
		  <el-col :span="4" v-for="(file, index) in msg.attachment">
				<div style="color:#84898a;padding:5px;margin-bottom:10px;background: #f8f8f8;padding-left:20px;padding-right:20px;">
					<el-row :gutter="20"  type="flex" align="middle">
					  <el-col :span="12" style="text-align: left;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><i class="el-icon-document"></i>{{file.name}}</el-col>
						<el-col :span="12" style="color:#409eff;cursor: pointer;text-align: right;"><span  @click="removeAttach" :data-index="index">删除</span></el-col>
					</el-row>
				</div>
		  </el-col>
		</el-row>
		<!--添加联系人对话框设置-->
		<el-dialog title="联系人" :visible.sync="contactDialogFormVisible" width="400px">
			<!--联系人列表-->
			...
			<div slot="footer" class="dialog-footer">
				...
				@click="doAddContact">添加</el-button>
		...
		<div v-if="type==='html'">
			<div v-if="way=='send'">
				<quill-editor v-model="msg.content"  style="height:260px;"
				  ref="myQuillEditor"
			...
			<div v-else>
				<quill-editor v-model="msg.content" style="height:160px;"
				  ref="myQuillEditor"
				  :options="editorOption" @blur="onEditorBlur($event)" @focus="onEditorFocus($event)"
				  @change="onEditorChange($event)">
				</quill-editor>
		...
		<!--底部按钮-->
		<el-row :gutter="20" type="flex" align="middle" style="margin-top:60px;padding-left:20px;padding-right:20px;">
		  <el-col :span="4">
				<div>
				<!--发件人-->
				<el-select v-model="msg.from" placeholder="请选择发件人" size="small"  :disabled="way=='reply'" @change="fromSelectChange">
						<el-option
		  ...
		  <el-col :span="8">
				<div>
					 <el-checkbox-group v-model="checkedList" @change="checkListChange">
						<el-checkbox :label="sendWay.label" v-for="sendWay in sendWayList"></el-checkbox>
					</el-checkbox-group>
				</div>
		  </el-col>
			<el-col :span="8">
		...
		<!--定时发送时间设置-->
		<el-row :gutter="20"  type="flex" align="middle" style="margin-top:10px;padding-left:20px;padding-right:20px;" v-if="checkedList.indexOf('定时发送') != -1">
			<el-col :span="3">
				<div>
		...
		<el-dialog title="邮件加密" :visible.sync="dialogFormVisible" width="30%">
			<el-form :model="secret">
				<el-form-item label="输入密码:">
					<el-input v-model="secret.one" autocomplete="off" show-password></el-input>
				</el-form-item>
				<el-form-item label="确认密码:">
					<el-input v-model="secret.two" autocomplete="off" show-password></el-input>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
...
<script>
	...
	export default {
      ...
      this.uploadUrl = localStorage.getItem('base_url')+'/upload';
			this.$axios({
			  method: 'get',
			  url: localStorage.getItem('base_url')+'/accountSign?account='+this.msg.from+'&way='+this.way+'&type='+this.type,
		...
		data(){
			return {
					...
					date:'',
					checkedList:[],
					sendWayList:[{label: '定时发送'},{label: '对邮件加密'},{label: '群发单显'}
					],
					dynamicTags:[],
					fileList:[],
					scheduleDate:{
				...
				this.$axios({
				  ...
				  'token':localStorage.token,
					...
					if(res.data.code == '200'){
						...
						_this.sendLoadingVisibled = false;
						_this.$message({type: 'error', message: '发送失败，请检查收件人地址是否正确!'});
					}
				});
			},
			subjectInputChange(){
        ...
        if(params.file.size >= 52428800){
          _this.$message({type: 'warning', message: '附件超过50M时大部分服务器不支持。'});
          return;
        }
        ...
        if(params.file.size + _requestSize >= 52428800){
          ...
          return;
			...
			fromSelectChange(value){
				...
				this.$axios({
				  method: 'get',
				  url: localStorage.getItem('base_url')+'/accountSign?account='+value+'&way='+_this.way+'&type='+_this.type,
				  headers:{'Content-Type': 'application/json', 'token':localStorage.token}
				}).then(function(res) {
					console.log(res.data)
					var _sign = res.data.data;
