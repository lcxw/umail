      ...
      <div class="ums-aside" style="width:400px;">
        <el-table :data="signs" :show-header="false">
          ...
          <el-table-column fixed="right" label="操作" width="50">
            <template slot-scope="scope">
              <el-button @click="newSign" type="text" size="small"
                >新建</el-button
              >
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="50">
            <template slot-scope="scope">
              <el-button @click="modifySign(scope.row)" type="text" size="small"
                >编辑</el-button
              >
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="50">
            <template slot-scope="scope">
              <el-button
                ...
                type="text"
                size="small"
                :disabled="accountNames.indexOf(scope.row.name) !== -1"
                >删除</el-button
              >
            </template>
          </el-table-column>
      ...
      <div class="ums-main">
        ...
        <el-row :gutter="2" type="flex" align="middle" style="margin-top:80px;">
          ...
          <el-col :span="6"
            ><div>
              <el-select
                v-model="accountSign.sendsign"
                size="small"
                @change="sendsignChange"
              >
                <el-option
                  v-for="sign in signs"
                  :key="sign.name"
          ...
          <el-col :span="5"
            ><div>
              <el-select
                v-model="accountSign.replysign"
                size="small"
                @change="replysignChange"
              >
                <el-option
                  v-for="sign in signs"
                  :key="sign.name"
        ...
        <div style="position:absolute;right:40px;bottom:40px;">
          <el-button
            ...
            >保存</el-button
          >
          <el-button size="small" style="width:80px;" @click="$router.go(-1)"
            >取消</el-button
          >
        </div>
      </div>
  ...
  methods: {
      ...
      this.$prompt("名称", "设置签名名称", {
        ...
        .then(({ value }) => {
          ...
          if (exist) {
            ...
            _this.$axios
              .post(localStorage.getItem("base_url") + "/signs", _sign, {
                headers: {
                  "Content-Type": "application/json",
                  token: localStorage.token,
                  type: "save"
    ...
    deleteSign(row) {
      ...
      this.$confirm("您确认要永久删除签名" + row.name + "吗？", "确认", {
        ...
        .then(() => {
          _this.$axios
            .post(localStorage.getItem("base_url") + "/signs", row, {
              headers: {
                "Content-Type": "application/json",
                token: localStorage.token,
                type: "delete"
